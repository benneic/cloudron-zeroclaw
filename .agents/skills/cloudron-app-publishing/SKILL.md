...
description: Distribute Cloudron apps via CloudronVersions.json. Covers cloudron versions init/add/update/revoke/list, cloudron build commands, manifest requirements for publishing, and distribution methods. Use when the user wants to publish, distribute, or share a Cloudron app package with others, or mentions CloudronVersions.json.
---
...
| Command | Purpose |
|---------|---------|
| `cloudron build reset` | Clear saved repository, image, and build info for the current directory |
...
- **No build service configured** — `cloudron build` uses the local Docker daemon. Requires Docker installed and `docker login` for registry auth.
- **Build service configured** (`cloudron build login`) — `cloudron build` sends source to the remote [Docker Builder App](https://docs.cloudron.io/packages/docker-builder/), which builds and pushes the image. Useful for slow upload networks or when more CPU/RAM is needed.
...
```bash
cloudron versions add
Added 1.0.0 in CloudronVersions.json
```
